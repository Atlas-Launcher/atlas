...
pub(crate) async fn session_from_ms_token(
  client_id: &str,
  ms_access_token: &str,
  refresh_token: Option<String>,
  fallback_refresh_token: Option<String>
) -> Result<AuthSession, String> {
  let xbl = xbox::authenticate(ms_access_token).await?;
  let xsts = xbox::xsts(&xbl.Token).await?;
  let uhs = xsts
    .DisplayClaims
    .xui
    .get(0)
  ...
  let mc = minecraft::login(&xsts.Token, &uhs).await?;

  let profile = minecraft::profile(&mc.access_token).await?;
  minecraft::verify_entitlements(&mc.access_token).await?;
...
pub(crate) async fn session_from_refresh(
  client_id: &str,
