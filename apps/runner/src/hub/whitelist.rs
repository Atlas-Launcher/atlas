...
impl InstanceConfig {
    pub async fn load(path: &Path) -> Result<Self> {
        let content = fs::read_to_string(path)
            .await
            .context("Failed to read instance.toml")?;
    ...
    pub async fn save(&self, path: &Path) -> Result<()> {
        ...
        let _: () = fs::write(path, content)
            .await
            .context("Failed to write instance.toml")?;
        Ok(())
    }
}
...
impl WhitelistSync {
    ...
    pub async fn sync(&self, pack_id: &str) -> Result<bool> {
        ...
        println!(
            "Syncing whitelist from Hub... (current etag: {:?})",
            current_etag
        );

        let (players, new_etag): (Vec<WhitelistEntry>, String) = self
            .hub
            .get_whitelist_with_version(pack_id, current_etag.as_deref())
            .await?;

        if players.is_empty() && current_etag.is_some() {
            // ETag matched, no update needed
            println!(
                "Whitelist is up to date (etag: {})",
                current_etag.as_ref().unwrap()
            );
            return Ok(false);
        }
