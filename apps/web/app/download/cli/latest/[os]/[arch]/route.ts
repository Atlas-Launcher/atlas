...
export async function GET(
  ...
  const limiter = rateLimit({
    id: `download-cli-binary-latest:${getClientIp(request)}`,
    limit: 120,
    windowMs: 60_000,
  });
  ...
  const asset = release.assets.find((entry) => entry.kind === "binary") ?? null;
  if (!asset) {
    return NextResponse.json({ error: `No CLI binary found for ${os}/${arch}.` }, { status: 404 });
  }
