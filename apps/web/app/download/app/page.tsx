...
export const metadata: Metadata = {
  ...
  description: "Download the latest Atlas Launcher build and update feed.",
};
...
const platformTargets = [
  ...
  },
  {
    ...
    extensions: [".dmg", ".pkg", ".app.tar.gz", ".zip"],
  },
  {
    id: "linux",
    label: "Linux",
        ...
        <div className="space-y-4 rounded-3xl border border-[var(--atlas-ink)]/10 bg-white/70 p-6 shadow-[0_24px_60px_rgba(16,20,24,0.1)]">
          ...
          <div className="rounded-2xl bg-[var(--atlas-cream)]/70 p-4">
            <p className="text-sm font-semibold text-[var(--atlas-ink)]">
              {release?.name ?? "Atlas Launcher"}
            </p>
            <p className="text-xs text-[var(--atlas-ink-muted)]">
              {formatTag(release?.tag_name, "launcher-v")} · {formatDate(release?.published_at)}
            </p>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="rounded-3xl border border-[var(--atlas-ink)]/10 bg-[var(--atlas-ink)] p-6 text-[var(--atlas-cream)]">
          ...
          <p className="mt-3 text-sm text-[var(--atlas-cream)]/70">
            Use the updater endpoint in your Tauri configuration. It always resolves the newest launcher release.
          </p>
          <div className="mt-4 space-y-2 text-xs">
            ...
            <p className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-[var(--atlas-cream)]/70">
              This endpoint selects the right update bundle for each platform and returns the required signature.
            </p>
          </div>
        </div>
      </section>
