...
export async function GET(request: NextRequest) {
  const limiter = rateLimit({
    id: `download-launcher-installer-auto:${getClientIp(request)}`,
    limit: 120,
    windowMs: 60_000,
  });
  ...
  if (!target) {
    const fallback = new URL("/download/app", request.url);
  ...
  const location = new URL(
    `/download/app/installer/latest/${target.os}/${target.arch}`,
    request.url,
  );
